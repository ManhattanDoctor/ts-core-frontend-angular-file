...
import { UploaderFile } from '../../lib/UploaderFile';

@Component({
    selector: 'file-image-crop',
    templateUrl: 'file-image-crop.component.html'
})
export class FileImageCropComponent extends IWindowContent implements ISerializable<string> {
    //--------------------------------------------------------------------------
    //
    //  Constants
    //
    ...
    @Input()
    public fileDropLabel: string = 'file.drop';

    @Input()
    public fileSelectLabel: string = 'file.select';
    ...
    protected commitFileProperties(): void {
        if (_.isNil(this.file)) {
            this.imageBase64 = null;
        }
    }

    protected commitBase64Properties(): void {
    ...
    public imageCropped(event: ImageCroppedEvent): void {
        this._imageWidth = event.width;
        this._imageHeight = event.height;
    ...
    public submit(): void {
        this.emit(FileImageCropComponent.EVENT_SUBMITTED);
    }
